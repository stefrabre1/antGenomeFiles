...
parser.add_argument('--filelist', help='a list of all files to be read')
...
parser.add_argument('--minAS', help='minimum quality threshold score to filter for (INT)', default='-12')
parser.add_argument('--maxXS', help='maximum duplication allowed (INT)', default='-25')
parser.add_argument('--verbose', help='print stats to standard output (terminal)', default='n')
...
# Welcome the user :)
print("The XS_AS_StatFilter.py script has launched!")
...
# read filenames from text file and remove any newline characters
with open(args.filelist, 'r') as names:
    fileNames = names.read().splitlines()

# open each file in the fileNames document and filter AS and XS data based on threshholds
for name in fileNames:
    
    filtered = os.path.join(args.outputdir, name)
    statsDoc = name + ".stats"
    stats = os.path.join(args.outputdir, statsDoc)
    ASList = []
    XSList = []
    temp = open(filtered, 'w+')
    statsFile = open(stats, 'w+')
    
    with open(name, 'r') as a_file:
        for line in a_file:
...
# indicate that all filtration is done
print("All XS_AS files have been filtered!")