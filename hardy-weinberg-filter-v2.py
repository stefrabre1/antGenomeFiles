...
parser.add_argument('--hom', help='percentage difference between homogenous and alternate (FLOAT)', default='0.2')
...
# determines if homogeneous alleles 1 and 2 are within a certain threshold of one another
# param: line, a line read from a file
# return: a boolean value, true if either homogeneous allele is not within the user specified threshold
def homoBalanced(line):
    column = getColumn(line, ALLELE_COL)
    firstHom = getAlleleCount(column, HOM1)
    secondHom = getAlleleCount(column, HOM2)
...
def badDataToFile(line, file):
    ...
    if hetAboveThreshold(line) and homoBalanced(line):
...
print("The homogenous threshold is %.2f" % HOM_THRESHOLD)
